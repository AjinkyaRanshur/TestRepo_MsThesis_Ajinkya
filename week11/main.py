...
def _load_illusion_dataset(config):
    """
    Load custom illusion dataset with stratified train/val/test splits.
    ...
    Returns:
        Tuple of (trainloader, validationloader, testloader)
    """
    from customdataset import SquareDataset
    import torchvision.transforms as transforms

    # Determine transform based on reconstruction dataset
    ...
    # Downsample random class to match basic shape count
    TARGET_RANDOM_TOTAL = len(basic_shape_indices["square"])
    NUM_SHOULD_SEE = len(random_indices_by_should_see)
    samples_per_should_see = TARGET_RANDOM_TOTAL // NUM_SHOULD_SEE
    ...
    for should_see_cls, indices in random_indices_by_should_see.items():
        ...
        sampled = rng.choice(indices, size=samples_per_should_see, replace=False)
    ...
    for cls_name, indices in basic_shape_indices.items():
        ...
        train_idx, val_idx = train_test_split(
            indices, test_size=0.3, random_state=config.seed
        )
        train_indices.extend(train_idx)
        val_indices.extend(val_idx)

    # Split random 70-30 STRATIFIED by should_see
    for should_see_cls, indices in random_indices_by_should_see.items():
        indices = np.array(indices)
        train_idx, val_idx = train_test_split(
            indices, test_size=0.3, random_state=config.seed
    ...
    # Sample all_in and all_out for test set
    rng = np.random.default_rng(config.seed)
    ...
    # Create test set = validation + sampled illusions
...
def _load_cifar10_dataset(config):
    """
    Load CIFAR-10 dataset for reconstruction training.

    Returns:
        Tuple of (trainloader, testloader, validationloader)
        Note: validationloader is None for CIFAR-10
    """
    import torchvision
    import torchvision.transforms as transforms
...
def _load_stl10_dataset(config):
    """
    Load STL-10 unlabeled dataset for reconstruction training.

    Uses 96x96 images with 80-20 train-test split.

    Returns:
        Tuple of (trainloader, testloader, validationloader)
        Note: validationloader is None for STL-10
    """
    import torchvision
    import torchvision.transforms as transforms
    from torch.utils.data import random_split

    # Transform: normalize using STL-10 normalization values
    transform = transforms.Compose([
        transforms.ToTensor(),
...
def train_test_loader(illusion_bool, config):
    """
    Creates train/validation/test dataloaders based on dataset type.

    Args:
        illusion_bool: Dataset type - "custom_illusion_dataset", "cifar10", or "stl10"
        config: Configuration object with dataset parameters

    Returns:
        Tuple of (trainloader, validationloader, testloader)
        Note: For CIFAR-10 and STL-10, the order is (trainloader, testloader, validationloader)
    """
    if illusion_bool == "custom_illusion_dataset":
        return _load_illusion_dataset(config)
    elif illusion_bool == "cifar10":
        return _load_cifar10_dataset(config)
    elif illusion_bool == "stl10":
        return _load_stl10_dataset(config)
    else:
        raise ValueError(f"Unknown dataset type: {illusion_bool}")
