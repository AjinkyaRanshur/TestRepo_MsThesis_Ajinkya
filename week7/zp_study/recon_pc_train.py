...
def recon_pc_training(net,trainloader,testloader,pc_train_bool,config):
    ...
    if pc_train_bool=="train":
        ...
        optimizer = optim.SGD(list(net.deconv4_fb.parameters())+list(net.deconv3_fb.parameters())+list(net.deconv2_fb.parameters())+list(net.deconv1_fb.parameters())+ list(net.conv1.parameters())+list(net.conv2.parameters())+list(net.conv3.parameters())+list(net.conv4.parameters()), lr=config.lr, momentum=config.momentum)
        loss_arr=[]
        ...
        for epoch in range(config.epochs):
            ...
            for batch_idx,batch in enumerate(trainloader):
                ...
                for i in range(config.timesteps):
                    ...
                    final_loss+=loss
                    train_recon_loss+=(loss_of_layers/4.0)
                    _,predicted=torch.max(output,1)
